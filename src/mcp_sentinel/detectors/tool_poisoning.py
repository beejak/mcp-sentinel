...
class ToolPoisoningDetector(BaseDetector):
            ...
            for category, patterns in self.text_patterns.items():
                for pattern in patterns:
                    ...
                    for match in matches:
                        if not self._is_false_positive_path(line, category, file_path):
    ...
    def _is_false_positive_path(self, line: str, category: str, file_path: Optional[Path] = None) -> bool:
        """Suppress false positives for path targeting patterns."""
        if category != "sensitive_path_targeting":
            return False
        # Suppress all sensitive-path hits in test/fixture files
        if file_path is not None and self._is_test_file(file_path):
            return True
