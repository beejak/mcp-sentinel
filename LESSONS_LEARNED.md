...
- **Config fingerprinting (Python equivalent):** Added deterministic scan-config fingerprint metadata via **`build_scan_config_metadata()`** (`core/config.py`) and emitted it in **`ScanResult.config["config_fingerprint"]`** for both scanner paths.
- **Tests:** Full **`pytest`** suite (**411** tests); **10** integration cases in **`tests/integration/test_static_detectors_integration.py`** plus config-fingerprint tests in **`tests/unit/test_config_fingerprint.py`**.
