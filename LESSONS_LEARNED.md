...
- **`PrototypePollutionDetector`:** **CWE-1321** (`VulnerabilityType.PROTOTYPE_POLLUTION`) — **`__proto__`** literals, **`.__proto__`**, **`setPrototypeOf`**; **`/* */`** stripped on the scan line before matching; **deep-merge + taint hint** patterns (`_.merge` / `defaultsDeep` / `lodash.*` near `req.body`, `JSON.parse`, `payload`, …). Wired into **Scanner** + **StaticAnalysisEngine** (9 default detectors).
