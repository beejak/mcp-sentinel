...
class ConfigSecurityDetector(BaseDetector):
    ...
    def _compile_patterns(self) -> dict[str, list[Pattern]]:
        ...
        return {
            ...
            "weak_auth": [
                ...
                re.compile(
                    r"password\s*:\s*['\"]?(?:admin|password|123|test)['\"]?", re.IGNORECASE
                ),
            ],
            ...
            "security_headers": [
                re.compile(
                    r"['\"]?X-Frame-Options['\"]?\s*:\s*['\"]?(?!DENY|SAMEORIGIN)[^'\"}\]]+",
                    re.IGNORECASE,
                ),
            ...
            "weak_secrets": [
                ...
                re.compile(
                    r"session_secret\s*=\s*['\"](?:secret|key|password)['\"]", re.IGNORECASE
                ),
                re.compile(
                    r"secret\s*:\s*['\"](?:secret|key|password|changeme|default|test|dev)['\"]",
                    re.IGNORECASE,
                ),
            ...
            "rate_limiting": [
                ...
                re.compile(r"['\"]rate_limit['\"]\s*:\s*None", re.IGNORECASE),
                re.compile(r"RATE_LIMIT\s*=\s*(?:None|False|0)", re.IGNORECASE),
                re.compile(r"rateLimit\s*:\s*False", re.IGNORECASE),
            ...
            "exposed_endpoints": [
                ...
                re.compile(
                    r"\.(?:get|post|put|delete|patch)\s*\(\s*['\"]/(?:debug|admin)(?:/[^'\"]*)?['\"]",
                    re.IGNORECASE,
                ),
                re.compile(
    ...
    def _is_likely_false_positive(
        self, line: str, matched_text: str, category: str, file_path: Path
    ) -> bool:
        """
        ...
        # For debug mode, skip typical local-only settings files (filename-based)
        if category == "debug_mode":
            ...
            ) or name_lower.endswith("_local.py"):
                return True
            if any(
                marker in line_lower
                for marker in [".env.local", "development.py", "settings_dev"]
            ):
                return True
