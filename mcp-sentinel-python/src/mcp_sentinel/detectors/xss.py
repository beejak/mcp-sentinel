...
class XSSDetector(BaseDetector):
    ...
    def _compile_patterns(self) -> dict[str, list[Pattern]]:
        ...
        return {
            ...
            "template_xss": [
                ...
                # Django safe() / mark_safe() — exclude Python "def safe():"
                re.compile(r"(?<!\bdef\s)mark_safe\s*\(\s*[^)]*\)"),
                re.compile(r"(?<!\bdef\s)(?<!\bfunction\s)\bsafe\s*\(\s*[^)]*\)"),
    ...
    async def detect(
        ...
        lines = content.split("\n")
        seen_match: set[tuple[int, str, str]] = set()
            ...
            for category, patterns in self.patterns.items():
                for pattern in patterns:
                    ...
                    for match in matches:
                        ...
                        if self._is_likely_false_positive(line, match.group(0), category):
                            continue

                        report_line = line_num
                        if (
                            file_type == "html"
                            and category == "event_handler_xss"
                            and line_num >= 2
                        ):
                            prev_line = lines[line_num - 2]
                            if "<" in prev_line and ">" not in prev_line:
                                report_line = line_num - 1

                        dedupe_key = (report_line, category, match.group(0))
                        if dedupe_key in seen_match:
                            continue
                        seen_match.add(dedupe_key)
    ...
    def _is_likely_false_positive(self, line: str, matched_text: str, category: str) -> bool:
        ...
        if category == "template_xss" and (
            "test" in str(line).lower() or "example" in str(line).lower()
        ):
            return True
